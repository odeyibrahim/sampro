(function () {
    ...
    var mapOrder = function(o) {
        return '<tr>' +
            ...
            '<td>₦' + esc((o.amount || 0).toFixed(0)) + '</td>' +
    ...
    function renderCustomersTable(customers) {
        ...
        tbody.innerHTML = customers.map(function(c) {
            return '<tr>' +
                ...
                '<td>₦' + (c.total_spent || 0).toFixed(0) + '</td>' +
        ...
        container.innerHTML = zones.map(function(z) {
            ...
            return '<div class="shipping-zone-row" data-zone-id="' + attr(z.id) + '"' + activeClass + '>' +
                '<div style="display:flex; align-items:center; gap:6px; flex:1; min-width:0; flex-wrap:wrap;">' +
                    ...
                    '</select>' +
                    '<input type="text" value="' + attr(z.currency || 'NGN') + '" data-field="currency" placeholder="Cur" style="width:50px; padding:4px 6px; border:1px solid var(--border-color, #ddd); border-radius:3px; font-size:11px;">' +
    ...
    async function addShippingZone() {
        ...
        if (!method) return;
        var currency = prompt('Currency (NGN, USD, GBP, EUR):', 'NGN');
        if (!currency) return;
        var cost = prompt('Shipping cost (' + currency + '):', '5000');
        if (cost === null) return;
